...
const updateDietaryPreference = asyncHandler(async (req, res) => {
    ...
    if (req.authUserData.userId !== userId) {
        return res.status(403).json({
            message: "Forbidden: You can only update your own preference"
        });
    }
...
const addUserMealPlan = asyncHandler(async (req, res) => {
    ...
    if (mealExists) {
        return res.status(400).json({
            message: "Meal is already in your meal plan"
        });
    }
...
const deleteUserMealPlan = asyncHandler(async (req, res) => {
    ...
    if (mealIndex === -1) {
        return res.status(404).json({
            message: `Meal ID ${mealId} not found in user's meal plan`
        });
    }
...
const addTrackerNonUser = asyncHandler(async (req, res) => {
    ...
    if (!validActivities.includes(activity) || !validGoals.includes(goal)) {
        return res.status(400).json({
            message: "Invalid activity or goal"
        });
    }
    ...
    // Success message
    res.json({
        message: "Macro data calculated successfully",
        data: {
            calories,
            protein,
            fat,
...
const addTracker = asyncHandler(async (req, res) => {
    try {
        ...
        console.error("Error in addTracker:", error); // Log the error
        res.status(500).json({
...
const updateTracker = asyncHandler(async (req, res) => {
    ...
    if (req.authUserData.userId !== userId) {
        return res.status(403).json({
            message: "Forbidden: You can only update your own tracker"
        });
    }
